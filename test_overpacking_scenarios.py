...
def run_overpacking_test():
    ...
    for i in range(num_episodes):
        ...
        while not done:
            action_to_take = None
            # Simple greedy agent: find the first available valid spot
            if env.mask_o0.any():
            ...
            if action_to_take:
                ...
                print(
                    f"  Step {info['counter']}: Placed a {env.box_creator.box_list[-1]} box at (o={o}, x={x}, y={y}). Done={done}"
                )
            else:
        ...
        # --- NEW: Print final heightmap ---
        print("  Final Bin State (Heightmap):")
        # Transpose for more intuitive (x,y) printing
