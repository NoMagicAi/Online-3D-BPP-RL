...
def init(module, weight_init, bias_init, gain=1):
    """
    A recursive weight initialization function that correctly handles
    both basic layers and composite modules, and also handles layers
    that may or may not have a bias.
    """
    # Initialize weights if they exist
    if hasattr(module, 'weight') and hasattr(module.weight, 'data'):
        weight_init(module.weight.data, gain=gain)
    
    # CORRECTED: Check if the bias exists before trying to initialize it
    if hasattr(module, 'bias') and module.bias is not None:
        bias_init(module.bias.data)
    
    # Recursively apply init to all children modules
    for _, child_module in module.named_children():
        init(child_module, weight_init, bias_init, gain)
