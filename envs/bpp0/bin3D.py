...
class PackingGame(gym.Env):
    ...
    def step(self, action):
        ...
        if mask_to_check[x_pos, y_pos]:
            ...
            variance_before = np.var(self.space.plain)
            ...
            variance_after = np.var(self.space.plain)
            air_pocket_penalty = sum_after - sum_before - box_volume
            r_flatness = variance_before - variance_after

            reward = volumetric_reward - 1.5 * (air_pocket_penalty/bin_volume) + (0 * r_flatness)
